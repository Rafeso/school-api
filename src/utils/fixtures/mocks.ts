...
export const dummyClass = (creationData?: Partial<ClassCreationType>) =>
	new Class({
		...
		teacher:
			creationData?.teacher === undefined ? teacherId : creationData.teacher,
	})
...
export const dummyTeacher = (creationData?: Partial<TeacherCreationType>) =>
	new Teacher({
		...
		hiringDate:
			creationData?.hiringDate ?? new Date("2010-10-10").toISOString(),
...
>(
	...
	findById: ctx.mock.fn(
		async (id: string) => methodReturns.findById ?? entityFactory({ id }),
	),
	...
	remove: ctx.mock.fn(
		async (_id: string) => methodReturns.remove ?? ctx.mock.fn(),
	),
	save: ctx.mock.fn(
